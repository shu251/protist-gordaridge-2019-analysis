...
The following analysis was performed in **R version 3.6.1**. All input files are available on the [Gorda Ridge GitHub ](https://github.com/shu251/protist-gordaridge-2019-analysis). 
...
plot_grid(plot_graze_trends + theme(legend.position = "none"),
          ...
          axis = c("tblr"), labels = c("a", "b", "c", ""), nrow = 1)
# dev.off()
```
...
```{r Plot carbon relationship raster, fig.height=3.5, fig.width=5}

# ten <- carbon_density %>% filter(ugC_L_perday > 10 & ugC_L_perday < 10.01)
...
```{r Modify data table}
x <- as.character(unique(flp_long_toplot$Vent.name))
# x
...
# Combine
t0_insitu_counts <- rbind(t0_flp, prok_avg_mod) #; t0_insitu_counts
```
...
```{r check library size, fig.height=4, fig.width=6}
# Decontam:
physeq_names
...
# See above
# load(file="data-input/GordaRidge-ASVtable-avg-19-08-2020.RData", verbose = T)
```
...
head(gr_counts_wreps_TAXA %>% select(LocationName, Taxa, SUM))
```
...
```{r Figure taxa with all replicates, fig.height = 9, fig.width = 13}
# svg("figs/barplot-taxalevel-wReps.svg", h = 9, w = 13)
barplot_lev2_wreps(gr_counts_wreps_TAXA)
# dev.off()
```

```{r Figure generation-all, fig.height = 7, fig.width = 9}
insitu <- c("sterivex", "SUPR")
rm <- c("Unassigned", "Opisthokonta-Other", "Opisthokonta-Fungi", "Opisthokonta-Metazoa")
...
```{r Export table for supplementary}
...
```{r Other taxonomic groups are not well resolved}
...
others <- c("Ciliates-Other", "Spirotrichea-Other", "Alveolata-Other", "Rhizaria-Other", "Stramenopiles-Other", "Stramenopiles-Bicoecea", "Amoebozoa", "Apusozoa", "Hacrobia-Other", "Archaeplastida-Other")
...
```{r plot tile, fig.width=8, fig.height=9}
...
# tile <- get_legend(tile_tax %+% subset(gr_relAbun_toheat, !(Taxa == "Unassigned-Eukaryote" | 
#                                            RES_COSMO == "Other" |
#                                            Taxa %in% rm)))
...
# ASV selective processes
```{r Import data if needed}
# load 18S data
# load("data-input/GR-countinfo-withASVdistribution.RData", verbose=T)
```

```{r Sort to isolate ASVs from in situ}
...
```{r Tile plot interaction total, fig.height=10, fig.width=12}
# ggplot(tax_interact_cor, aes(x = PROK, y = EUK, fill = SUM_COUNT)) +
#   geom_tile(stat = "identity", color = "black") +
#   scale_fill_gradient(low = "#dadaeb", high = "#4a1486") +
#   theme_bw() +
...
```{r Alluvial plot, fig.height=18, fig.width=25}
...
# Excess
## Explore putative enrichment
```{r Subset only ASVs in situ and in grazing}
load("data-input/GR-countinfo-withASVdistribution.RData", verbose = T)
...
venti <- ggplot(dflatte, aes(x = x_num, fill = LEVEL2ORDER, y = SUM)) +
  geom_area(aes(fill = LEVEL2ORDER), position = "stack", stat = "identity", color = "black") +
  scale_x_discrete(expand = c(0,0)) +
  scale_fill_manual(values = level2color) +
...
```{r}
sirvent <- ggplot(dfsir, aes(x = x_num, fill = LEVEL2ORDER, y = SUM)) +
  geom_area(aes(fill = LEVEL2ORDER), position = "stack", stat = "identity", color = "black") +
  scale_x_discrete(expand = c(0,0)) +
  scale_fill_manual(values = level2color) +
  # scale_color_manual(values = level2color) +
...
```{r}
edward <- ggplot(dfed, aes(x = x_num, fill = LEVEL2ORDER, y = SUM)) +
  geom_area(aes(fill = LEVEL2ORDER), position = "stack", stat = "identity", color = "black") +
  scale_x_discrete(expand = c(0,0)) +
  scale_fill_manual(values = level2color) +
  # scale_color_manual(values = level2color) +
